...
def post(request, pk):
    blog_post = blogs.objects.get(id = pk)
...
def dbpush(request):
    if request.method == 'POST':
        ...
        if blogs.objects.filter(title = title).exists():
            messages.error(request, 'Title already used.')
            return redirect('educational')
        elif blogs.objects.filter(blog = blog).exists():
            messages.error(request, 'This title has been chosen by another doctor, please select another topic')
            return redirect('educational')
        else:
            newobj = blogs.objects.create(name = name, title = title, blog = blog)
            newobj.save()
...
#for showing signup/login button for admin_add_appointment
